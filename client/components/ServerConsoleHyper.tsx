...
const ActivityLogWidget = () => {
    ...
    const { data, error } = useSWR(`/api/client/servers/${uuid}/activity?include=actor&sort=-timestamp&page=1`, async (url) => {
    ...
    return (
        <div className="flex flex-col relative pl-4 space-y-4 p-4 custom-scrollbar">
            ...
            {data.slice(0, 15).map((activity: any, index: number) => {
                ...
                <div key={index} className="relative flex items-start group">
                    ...
                    <img 
                        src={`https://minotar.net/avatar/${activity.attributes.relationships?.actor?.attributes?.username || activity.attributes.user?.username || 'Steve'}/32.png`} 
                        alt="avatar" 
            ...
            <div className={isEditing ? '[&>.react-grid-item]:border [&>.react-grid-item]:border-dashed [&>.react-grid-item]:border-purple-500 [&>.react-grid-item]:bg-purple-500/5' : ''}>
                ...
                >
                    ...
                    <div key="console" className={`flex flex-col bg-gray-800/80 border border-indigo-500/30 rounded-xl overflow-hidden shadow-lg ${isMaximized ? 'fixed inset-4 z-[9999] shadow-2xl shadow-black h-auto bg-gray-900' : 'h-full'} ${isEditing && !isMaximized ? 'cursor-move' : ''}`}>
                        ...
                        `}} />
                        <div className="pterodactyl-console-wrapper flex-1 relative min-h-0 h-full w-full [&>div]:h-full">
                            <Spinner.Suspense>
                                <Console />
                            </Spinner.Suspense>
                        </div>
